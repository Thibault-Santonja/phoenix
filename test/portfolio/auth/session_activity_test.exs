defmodule Portfolio.Auth.SessionActivityTest do
  ...
  describe "session activity tracking" do
    ...
    test "session activity updates multiple times" do
      ...
      assert DateTime.compare(activity3, activity2) == :gt
    end
    ...
    test "session activity tracks user engagement" do
      ...
      # Initial activity
      activities = [session.last_activity_at | activities]

      # Activity 1: 1 second later
      Process.sleep(1100)
      {:ok, session} = Auth.update_session_activity(session)
      activities = [session.last_activity_at | activities]

      # Activity 2: 1 second later
      Process.sleep(1100)
      {:ok, session} = Auth.update_session_activity(session)
      activities = [session.last_activity_at | activities]

      # Assert: All activities have increasing timestamps
      activities = Enum.reverse(activities)

      assert DateTime.compare(Enum.at(activities, 1), Enum.at(activities, 0)) == :gt
      assert DateTime.compare(Enum.at(activities, 2), Enum.at(activities, 1)) == :gt
    end
    ...
    test "multiple sessions track activity independently" do
      ...
      assert DateTime.compare(updated_session1.last_activity_at, session1.last_activity_at) ==
               :gt
    ...
    test "delete_all_user_sessions removes all sessions" do
      ...
      assert :ok = Auth.delete_all_user_sessions(user)
