defmodule PortfolioWeb.Admin.AlbumLiveTest do
  ...
  describe "Album Form Component Events" do
    setup :register_and_log_in_user

    test "validates form via component target", %{conn: conn} do
      {:ok, live, _html} = live(conn, ~p"/admin/albums/new")

      # The form has phx-target={@myself} which targets the component
      # Validate with empty title should show validation in form
      html =
        live
        |> form("#album-form", album: %{title: "", type: "landscape"})
        |> render_change()
    ...
    test "shows error on save with invalid data", %{conn: conn} do
      {:ok, live, _html} = live(conn, ~p"/admin/albums/new")
    ...
    test "validates existing album form", %{conn: conn} do
      album = create_album(title: "Test Album")

      {:ok, live, _html} = live(conn, ~p"/admin/albums/#{album.id}/edit")

      # Validate change on existing album
      html =
        live
        |> form("#album-form", album: %{title: "Changed Title"})
        |> render_change()

      assert html =~ "Changed Title" or html =~ "form"
    end

    test "shows error on edit with invalid changeset", %{conn: conn} do
      album = create_album()
      ...
      assert html =~ "form" or html =~ "album" or html =~ "error" or html =~ "required"
    end
  end
  ...
  describe "Album Edit Photo Management" do
    ...
    test "can reprocess a photo", %{conn: conn} do
      album = create_album()
      photo = create_photo(album: album)

      {:ok, live, _html} = live(conn, ~p"/admin/albums/#{album.id}/edit")

      html = render_click(live, "reprocess_photo", %{"id" => photo.id})

      # Should show reprocessing message
      assert html =~ "reprocess" or html =~ "traitement" or html =~ album.title or
               html =~ "relancé"
    end

    test "can save photo order after reordering", %{conn: conn} do
      album = create_album()
      photo1 = create_photo(album: album, title: "Photo 1")
      photo2 = create_photo(album: album, title: "Photo 2")
      ...
      # Reorder photos (swap order)
      render_click(live, "reorder_photos", %{"photo_ids" => [photo2.id, photo1.id]})

      # Save the new order
      html = render_click(live, "save_photo_order", %{})

      # Should confirm order was saved
      assert html =~ "order" or html =~ "ordre" or html =~ album.title or html =~ "saved" or
               html =~ "enregistré"
    end

    test "upload area is present", %{conn: conn} do
      album = create_album()

      {:ok, _live, html} = live(conn, ~p"/admin/albums/#{album.id}/edit")

      # Upload area should be present
      assert html =~ "upload" or html =~ "photo" or html =~ "drop" or html =~ album.title
    end

    test "handles save_photo with invalid data", %{conn: conn} do
      album = create_album()
      photo = create_photo(album: album)
      ...
      # Try to save with invalid data (empty title if required)
      html = render_click(live, "save_photo", %{"photo" => %{"title" => ""}})

      # Should show form with errors or stay in modal
      assert html =~ "photo" or html =~ "form" or html =~ album.title
    end
  end

  describe "Album Edit - FormComponent integration" do
    setup :register_and_log_in_user

    test "receives saved message from form component", %{conn: conn} do
      album = create_album(title: "Original Title")

      {:ok, live, _html} = live(conn, ~p"/admin/albums/#{album.id}/edit")

      new_title = "Updated Via Form #{System.unique_integer([:positive])}"

      # Submit the form which triggers FormComponent save
      _result =
        live
        |> form("#album-form", album: %{title: new_title})
        |> render_submit()
