defmodule PortfolioWeb.Components.EmailField do
  @moduledoc """
  The `PortfolioWeb.Components.EmailField` module provides a customizable email input field
  ...
  def email_field(%{floating: floating} = assigns) when floating in ["inner", "outer"] do
    ~H"""
    <div class={[color_variant(@variant, @color, @floating), rounded_size(@rounded), border_class(@border, @variant), size_class(@size), space_class(@space), @ring && "focus-within:[&_.email-field-wrapper]:ring-[0.03rem]", @class]}>
      <div :if={@description} class={@description_class}>
        {@description}
      </div>
      <div class={["email-field-wrapper flex w-full flex-nowrap transition-all duration-200 ease-in-out", @errors != [] && "email-field-error", @field_wrapper_class]}>
        <div
          :if={@start_section}
  ...
  def email_field(assigns) do
    ...
    <div class={[color_variant(@variant, @color, @floating), rounded_size(@rounded), border_class(@border, @variant), size_class(@size), space_class(@space), @ring && "focus-within:[&_.email-field-wrapper]:ring-[0.03rem]", @class]}>
      ...
      <div class={["email-field-wrapper flex flex-nowrap items-center overflow-hidden transition-all duration-200 ease-in-out", @errors != [] && "email-field-error", @field_wrapper_class]}>
        <div
          :if={@start_section}
          class={["ps-2 flex shrink-0 items-center justify-center", @start_section[:class]]}
        >
