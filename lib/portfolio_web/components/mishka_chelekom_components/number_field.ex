defmodule PortfolioWeb.Components.NumberField do
  @moduledoc """
  The `PortfolioWeb.Components.NumberField` module provides a versatile and customizable
  ...
  def number_field(%{floating: floating} = assigns) when floating in ["inner", "outer"] do
    ~H"""
    <div class={[color_variant(@variant, @color, @floating), rounded_size(@rounded), border_class(@border, @variant), size_class(@size), space_class(@space), @ring && "focus-within:[&_.number-field-wrapper]:ring-[0.03rem]", @class]}>
      <div :if={@description} class={@description_class}>
        {@description}
      </div>
      <div class={["number-field-wrapper flex w-full flex-nowrap transition-all duration-200 ease-in-out", @errors != [] && "number-field-error", @field_wrapper_class]}>
        <div
          :if={@start_section}
        ...
        <div class="z-[2] relative w-full">
          <input
            ...
            id={@id}
            value={@value}
            class={["z-[2] block w-full px-2 pt-2.5 pb-1 placeholder:text-transparent focus:ring-0 disabled:opacity-80", "text-[16px] peer appearance-none border-0 bg-transparent focus:outline-none sm:font-inherit", @controls == "fixed" && ":[&:-webkit-outer-spin-button]:opacity-100 :[&:-webkit-inner-spin-button]:opacity-100", @controls == "hide" && "[appearance:textfield] :[&:-webkit-outer-spin-button]:appearance-none :[&:-webkit-inner-spin-button]:appearance-none", @input_class]}
            placeholder=" "
            {@rest}
          />
  ...
  def number_field(assigns) do
    ...
    <div class={[color_variant(@variant, @color, @floating), rounded_size(@rounded), border_class(@border, @variant), size_class(@size), space_class(@space), @ring && "focus-within:[&_.number-field-wrapper]:ring-[0.03rem]", @class]}>
      ...
      <div class={["number-field-wrapper flex flex-nowrap items-center overflow-hidden transition-all duration-200 ease-in-out", @errors != [] && "number-field-error", @field_wrapper_class]}>
        <div
          :if={@start_section}
          class={["ps-2 flex shrink-0 items-center justify-center", @start_section[:class]]}
        >
        ...
        <input
          ...
          class={["block w-full flex-1 appearance-none px-2 py-1 text-sm disabled:opacity-80", "border-0 bg-transparent focus:outline-none focus:ring-0", @controls == "fixed" && ":[&:-webkit-outer-spin-button]:opacity-100 :[&:-webkit-inner-spin-button]:opacity-100", @controls == "hide" && "[appearance:textfield] :[&:-webkit-outer-spin-button]:appearance-none :[&:-webkit-inner-spin-button]:appearance-none", @input_class]}
          {@rest}
        />
