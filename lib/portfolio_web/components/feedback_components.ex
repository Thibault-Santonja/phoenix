defmodule PortfolioWeb.FeedbackComponents do
  @moduledoc """
  Feedback UI components for user notifications and modals.

  This module provides components for displaying feedback to users:
            ...
            >
              <div class="absolute top-6 right-5">
                <button
                  ...
                  aria-label={gettext("components.modal.close")}
                >
                  <PortfolioWeb.CoreComponents.icon name="hero-x-mark-solid" class="h-5 w-5" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        <PortfolioWeb.CoreComponents.icon
          :if={@kind == :info}
  ...
  def flash_group(assigns) do
    ~H"""
    <div id={@id}>
      <.flash kind={:info} title={gettext("components.flash.success")} flash={@flash} />
      <.flash kind={:error} title={gettext("components.flash.error")} flash={@flash} />
      <.flash
        id="client-error"
        kind={:error}
      ...
      >
        ...
        <PortfolioWeb.CoreComponents.icon name="hero-arrow-path" class="ml-1 h-3 w-3 animate-spin" />
      </.flash>
